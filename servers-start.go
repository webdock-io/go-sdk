...
type TurnServerOnOptions struct {
	Slug string
}
...
*/
func (w *Webdock) TurnServerOn(options TurnServerOnOptions) (string, error) {
	URL := url.URL{
		Scheme: "https",
		Host:   w.BASE_URL,
		Path:   fmt.Sprintf(`v1/servers/%s/actions/start`, options.Slug),
	}
